...
async function technocore(payload: Record<string, string>) {
  return request("/api/technocore", {
    method: "POST",
...
export default function Home() {
  ...
  const signedPost = async (room: string, text: string) => {
    ...
    const response = await technocore({ kind: "signed-message", did: key.did, room, nonce, sig: signPayload(key, room, nonce, text), text }) as { posted?: { seq?: number } };
  ...
  return <main className="shell">
    ...
    <section className="safety"><span>SECURITY PROTOCOL</span><p>Never paste a private key into X, GitHub, a form, or a screenshot. Keep <code>agent_key.json</code> offline and backed up. This app relays only public, already-signed messages to Technocore—never your private key. A server error does not confirm a post—retry only after checking Technocore.</p></section>
  </main>;
}
